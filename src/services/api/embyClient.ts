...
export class EmbyApiClient {
  ...
  constructor(config: ApiConfig) {
    this.baseURL = config.serverUrl.replace(/\/$/, '') // 移除末尾的斜杠
    ...
    this.axiosInstance.interceptors.request.use(
      (config) => {
        ...
        // 只有在不跳过认证且有 token 时才添加 X-Emby-Token
        if (!shouldSkipAuth && this.accessToken) {
          config.headers['X-Emby-Token'] = this.accessToken
        }
  ...
  public setServerUrl(url: string): void {
    ...
    this.axiosInstance.defaults.baseURL = `${this.baseURL}/emby`
  }
